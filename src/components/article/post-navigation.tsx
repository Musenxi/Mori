...
function NavBlock({ label, post }: { label: string; post?: NormalizedPost }) {
  ...
  return (
    <>
      ...
      <Link href={post.redirect || `/post/${post.slug}`} prefetch={!post.redirect ? false : undefined} target={post.redirect ? "_blank" : undefined} rel={post.redirect ? "noopener noreferrer" : undefined} className="block py-5">
